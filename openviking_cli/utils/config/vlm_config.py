...
class VLMConfig(BaseModel):
            ...
            if isinstance(providers, dict):
                ...
                for name, config in providers.items():
                    ...
                    if normalized_name is None:
                        raise ValueError("VLM provider config contains an empty provider name")
        ...
        if self._has_any_config():
            ...
            if provider_name == "openai-codex":
                ...
                if not self._get_effective_api_key() and not has_codex_auth_available():
                    raise ValueError(
                        "VLM configuration requires Codex OAuth credentials in "
                        "~/.openviking/codex_auth.json or an importable Codex CLI auth file"
                    )
    ...
    def _migrate_legacy_config(self):
        ...
        if self.api_key and self.provider:
            ...
            if "stream" not in self.providers[self.provider]:
    ...
    def _get_provider_config_by_name(self, provider_name: str) -> Dict[str, Any]:
        ...
        if env_api_key and not config.get("api_key"):
    ...
    def _match_provider(self, model: str | None = None) -> tuple[Dict[str, Any] | None, str | None]:
        ...
        del model
        if self.provider:
            return self._get_provider_config_by_name(self.provider), self.provider

        if self.default_provider:
            return self._get_provider_config_by_name(self.default_provider), self.default_provider
