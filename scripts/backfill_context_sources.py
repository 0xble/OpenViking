...
def _iter_records(manager: VikingDBManager, batch_size: int) -> Iterable[Dict[str, Any]]:
...
async def backfill_sources(batch_size: int, dry_run: bool) -> Dict[str, Any]:
    ...
    try:
        for record in _iter_records(manager, batch_size):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Number of records to fetch per page while scanning the collection.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    args = parse_args()
    result = asyncio.run(backfill_sources(batch_size=args.batch_size, dry_run=args.dry_run))
