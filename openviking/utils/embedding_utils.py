...
def _embedding_max_input_tokens(embedding_cfg: object) -> int:
    value = getattr(embedding_cfg, "max_input_tokens", None)
    if value is not None:
        return int(value or 4096)

    legacy_max_chars = getattr(embedding_cfg, "max_input_chars", None)
    if legacy_max_chars is not None:
        return max(1, math.ceil(int(legacy_max_chars or 1) / 4))
...
async def vectorize_file(
    ...
    try:
        ...
        max_input_chars = getattr(embedding_cfg, "max_input_chars", None)
        max_input_tokens = _embedding_max_input_tokens(embedding_cfg)
            ...
            if summary and effective_text_source in {"summary_first", "summary_only"}:
                ...
                try:
                    ...
                    if max_input_chars is not None:
                        ...
                        if max_chars <= 0:
                            content = _EMBEDDING_TRUNCATION_SUFFIX.lstrip()
                        elif len(content) > max_chars:
                            content = content[:max_chars].rstrip() + _EMBEDDING_TRUNCATION_SUFFIX
                    else:
